...
int main() {
    ...
    using v_t = scratch::variant<B, A>;
    etl::Vector<v_t, 4> p;
    p.push_back(v_t(scratch::in_place_index_t<1>{}, A{32_B}));
    p.push_back(v_t(scratch::in_place_index_t<0>{}, B{32_B}));
...
//    p.push_back(B{62_B});
    ...
    while(true) {
        std::byte x{};
        for(auto&& i : p) {
            visit([&]<typename P>(P& v){
                        v.store();
                        x |= v.get();
                    }, i);
        }
        //        x1.set(x);
        visit([&]<typename V>(V& v){
