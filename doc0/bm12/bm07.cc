...
template<typename PGroup, typename ValueType, typename Trans = TransitionsPerDent<4>, bool useInterrupts = false>
...
private:
    static void update() {
        ...
        if (diff & 0b01) {  // one-step change
            mLastState = newState;                    
            mValue += (diff & 0b10) - 1;   // map: 0,2 -> -1,1
        }
    }
...
using rot = RotaryEncoder<rotaryPins, int32_t, TransitionsPerDent<2>, true>;
#else
using rot = RotaryEncoder<rotaryPins, int32_t, TransitionsPerDent<2>, false>;
#endif
