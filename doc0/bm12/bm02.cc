    ...
    static void periodic() {
        ...
        if (diff & 0b01) {                // bit 0 = value (1)
            ...
            if constexpr(useInterrupts) {
                volatile_access(mValue) += (diff & 0b10) - 1;   // bit 1 = direction (+/-)
            }
            else {
               mValue += (diff & 0b10) - 1;   // bit 1 = direction (+/-)
            }
        }
    }
...
using rot = RotaryEncoder<pa5, pa6, uint32_t, TransitionsPerDent<2>, true>;
#else
using rot = RotaryEncoder<pa5, pa6, uint32_t, TransitionsPerDent<2>, false>;
#endif
