...
//                        std::cout << "c\n";
    ...
    namespace Sensor {
        enum class Id : uint8_t {Recv = 0x80, General = 0x8d};
        
        class Query {
        public:
            explicit Query(Id id) : mId(id) {
            }
            bool transmit(Serial& port) {
                if (!port) {
                    return false;
                }
                std::array<uint8_t, 2> q = {};
                q[0] = startByte;
                q[1] = static_cast<uint8_t>(mId);
                
                auto l = port.sendWithDelay(q, 3000);
                if (l < 0) {
                    return false;
                }
                return l == 2;
        ...
        private:
            inline static constexpr uint8_t startByte = 0x80;
            const Id mId;
        };
        ...
        class Sensor {
        public:
            explicit Sensor(const std::string& portName) : mPort(portName) {
            }
            bool send(Hott::Sensor::Id id) {
                if (!mPort) {
                    return false;
                }
                else {
                    Hott::Sensor::Query query(id);
                    query.transmit(mPort);
                }
                return true;
            }
            Hott::Sensor::Message receive() {
                Hott::Sensor::Message message;
                if (mPort) {
                    if (message.receive(mPort)) {
                        return message;
                    }              
...
int main() {
    Hott::Sensor::Sensor sensor("/dev/ttyUSB0");
    ...
    while(true) {
        sensor.send(Hott::Sensor::Id::General);
        useconds_t d = 1000 * 900;
        if (usleep(d) < 0) {
            perror("usleep");
        }
    }    
