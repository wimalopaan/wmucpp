...
struct Bandpass {
    Bandpass() = delete;
    static inline IQ_Bands process(const IQ v) {
...
struct Square {
    ...
    static inline IQ_Bands process(const IQ_Bands v) {
...
struct Thresh {
    ...
    static inline IQ_Bit process(const IQ_Bands v) {
        IQ_Bit b;
...
struct Stats {
    Stats() = delete;
    static inline void process(const IQ v) {
        ++numberOfSamples;
        min = std::min(min, v.i);
        min = std::min(min, v.q);
        max = std::max(max, v.i);
...
struct Protocoll {
    ...
    struct ByteStuff {
        ByteStuff() = delete;
        static inline bool process(const bool b) {
    ...
    static inline void process(const IQ_Bit b) {
