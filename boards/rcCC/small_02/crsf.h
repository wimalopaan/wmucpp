...
template<auto Size, typename Out, typename Timer>
struct ExpMultiRateFilter {
    static inline void init() {
        for(auto& f : filter) {
            f.factor(0.2);
        }
    }
    static inline void set(const auto& in) {
        for(uint8_t i = 0; i < Size; ++i) {
            inputValues[i] = in[i];
        }
    }
    static inline constexpr External::Tick<Timer> updateTicks{20ms};
    static inline void ratePeriodic() {
        ++mStateTick;
        mStateTick.on(updateTicks, []{
            for(uint8_t i = 0; i < Size; ++i) {
                filter[i].process(inputValues[i]);
                Out::set(i, (int8_t)filter[i].value());
            }
        });
    }
    private:
    static inline External::Tick<Timer> mStateTick;
    static inline std::array<int8_t, Size> inputValues;
    static inline std::array<Dsp::ExpMean<>, Size> filter;
};
...
struct CrsfCallback {
    ...
    using sbus2 = Config::sbus2;

    using sbusfilter = ExpMultiRateFilter<16, sbus2, timer>;
    ...
    static inline void command(const std::array<uint8_t, L>& payload) {
        if (payload[0] == (uint8_t)RC::Protokoll::Crsf::Address::Controller) {
            ...
            if (payload[2] == (uint8_t)RC::Protokoll::Crsf::CommandType::CC) {
                if (payload[3] == (uint8_t)RC::Protokoll::Crsf::CcCommand::SetChannel) {
                    const int8_t ch0 = (int8_t)payload[4];
                    IO::outl<trace>("SetChannel: ch0: ", ch0);

                    // todo: exponential filtering (in sbus generator ...)

                    sbusfilter::set((int8_t*)(&payload[0] + 4));

                    // for(uint8_t i = 0; i < 16; ++i) {
                    //     sbus2::set(i, (int8_t)payload[i + 4]);
                    // }
                }
            }
        }
