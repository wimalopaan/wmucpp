...
struct Devices<Foc01, Config, MCU> {
	...
	using adc1DmaChannel = Mcu::Stm::Dma::Channel<dma1, 5, MCU>;
	using adc2DmaChannel = Mcu::Stm::Dma::Channel<dma1, 6, MCU>;
    ...
    using slave = void;
    // Usart 2: Debug
    using debugtx = Mcu::Stm::Pin<gpiob, 10, MCU>;
    struct DebugConfig;
    using debug = SerialBuffered<3, DebugConfig, MCU>;
    struct DebugConfig {
        using pin = debugtx;
...
#else
    using debug = void;
    using slavetx = Mcu::Stm::Pin<gpiob, 10, MCU>;
    using slaverx = Mcu::Stm::Pin<gpiob, 11, MCU>;
#endif

    // Leds
    using ledPin1 = Mcu::Stm::Pin<gpiob, 12, MCU>;
    using led1 = External::Blinker<ledPin1, systemTimer>;

    using ledPin2 = Mcu::Stm::Pin<gpiob, 13, MCU>;
    using led2 = External::Blinker<ledPin2, systemTimer>;

#ifdef USE_I2C
    using tp = void;
#else
    // Testpin
    using tp = Mcu::Stm::Gpio::ActiveHigh<Mcu::Stm::Pin<gpioa, 15, MCU>>;
#endif

    // using mco = Mcu::Stm::Pin<gpioa, 8, MCU>;

	struct PwmConfig;
	using pwm = Mcu::Stm::V3::Pwm::ThreePhases<1, PwmConfig>;
	using pwm1 = Mcu::Stm::Pin<gpioa, 8, MCU>;
	using pwm2 = Mcu::Stm::Pin<gpioa, 9, MCU>;
	using pwm3 = Mcu::Stm::Pin<gpioa, 10, MCU>;
	
    // SPI 2:
    using spi_cs = Mcu::Stm::Pin<gpiof, 0, MCU>; 
    using spi_miso = Mcu::Stm::Pin<gpiob, 14, MCU>;
    using spi_mosi = Mcu::Stm::Pin<gpiob, 15, MCU>;
    using spi_clk = Mcu::Stm::Pin<gpiof, 1, MCU>;

    using drv8317_nfault = Mcu::Stm::Pin<gpiob, 4, MCU>;
    using drv8317_nsleep = Mcu::Stm::Pin<gpiob, 5, MCU>;
    using drv8317_pwmEnable = Mcu::Stm::Pin<gpioa, 12, MCU>;
    using drv8317_mode = Mcu::Stm::Pin<gpioa, 6, MCU>;
	...
	using soa = Mcu::Stm::Pin<gpioa, 7, MCU>; // OPAMP1/2 VINP (1)
	using sob = Mcu::Stm::Pin<gpiob, 0, MCU>; // OPAMP2/3 VINP (2)
	using soc = Mcu::Stm::Pin<gpioa, 1, MCU>; // OPAMP1/3_VINP (3)
    ...
    using components = StandardComponents<trace, debug, crsf, led1, led2, ams5048, servo>;
    ...
    static inline void init() {
		...
		drv8317_nsleep::template dir<Mcu::Output>();
		drv8317_nsleep::set();
		drv8317_pwmEnable::template dir<Mcu::Output>();
		drv8317_pwmEnable::set();
