...
struct PatternGenerator {
    static inline auto ramps = []{
        std::array<External::PCA9745::Registers::Ramp, 2> d;
        d[0].rate = 0x81_B; // ramp up
        d[0].step = 0x04_B; // 0,5ms * 1
        d[0].hold = 0x00_B; // no hold 
        d[0].iref = 255_B;
        d[1].rate = 0x41_B; // ramp down
        d[1].step = 0x04_B; // 0,5ms * 1
        d[1].hold = 0x00_B; // no hold 
        d[1].iref = 255_B;
        return d;
    }();
    
    static inline void invert(etl::uint_ranged_circular<uint8_t, 0, 1> g) {
        ramps[g].rate ^= 0xc0_B;
    }
    
    static constexpr External::Tick<Timer> seqTimeoutTicks{500_ms};
    ...
    using circ8_t = etl::uint_ranged_circular<uint8_t, 0, 7>;
...
private:
    ...
    static inline void sequenceNext() {
        ...
        invert(mGrp1);
        invert(mGrp2);
        PCA::pushGradation(ggroup_t{0, etl::RangeCheck<false>{}}, ramps[0]);        
        PCA::pushGradation(ggroup_t{1, etl::RangeCheck<false>{}}, ramps[1]);        
        
        PCA::group(mOut1.toRanged(), mGrp1);
        PCA::enableGradation(mOut1.toRanged(), true);
        PCA::out(mOut1.toRanged(), PCA::LedMode::On);

        PCA::out(mOut3.toRanged(), PCA::LedMode::Off);
        PCA::enableGradation(mOut3.toRanged(), false);
        
        PCA::gradationStartSingle(0b0000'1010_B);
...
//        PCA::pushGradation(ggroup_t{2, etl::RangeCheck<false>{}}, ramps[0]);        
//        PCA::pushGradation(ggroup_t{3, etl::RangeCheck<false>{}}, ramps[0]);        
    ...
    static inline circ8_t mOut1;
    static inline circ8_t mOut2;
    static inline circ8_t mOut3;
    static inline etl::uint_ranged_circular<uint8_t, 0, 1> mGrp1;
    static inline etl::uint_ranged_circular<uint8_t, 0, 1> mGrp2;
