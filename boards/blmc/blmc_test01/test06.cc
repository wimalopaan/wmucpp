...
//            pwm<h2>();
    ...
    inline static bool rising = false;
    ...
    inline static bool zeroCrossed2() {
        lastAC1 = lastAC2;
        lastAC2 = !acv();
        if (rising & lastAC1 & lastAC2) {
            return true;
        }        
        if (!rising & !lastAC1 & !lastAC2) {
            return true;
...
struct Controller {
    ...
    static inline RampPoint ramp_start = {10000_us, 5_ppc};
    static inline RampPoint ramp_end   = {7000_us, 5_ppc};
    static inline uint8_t ramp_steps = 30;
    ...
    inline static void run() {
        spin();
        if ((mState != State::Off) & (mState != State::ClB)) {
...
int main() {
    ...
    // watchdog: alles aus
    {
        ...
        std::outl<terminal>("BL-Ctrl (mod) V06"_pgm);
