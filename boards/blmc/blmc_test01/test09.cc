...
struct Controller {
    ...
    enum class State : uint8_t {Off, Align, RampUp, ClosedLoop, ClosedLoopComDelay, ClosedLoopCommute, ClosedLoopError};
    ...
    static inline auto pwmStart = FixedPoint<uint16_t, 8>{(double)std::expand(6_ppc, uint8_t{0}, pwm_max)};
...
//    static inline auto pwmStart = FixedPoint<uint16_t, 8>{(double)std::expand(5_ppc, uint8_t{0}, pwm_max)};
    ...
    static inline constexpr uint16_t alignValue = 300_ms * fTimer;
...
int main() {
    ...
    
    {
        ...
        std::outl<terminal>("BL-Ctrl (mod) V09"_pgm);
...
//                    std::outl<terminal>("f:"_pgm, controller::period);
                    std::outl<terminal>("v:"_pgm, adc_ol::mVoltage);
                    std::outl<terminal>("t:"_pgm, adc_ol::mTemp);
                    std::outl<terminal>("c:"_pgm, adc_ol::mCurrent);
                    std::outl<terminal>("n:"_pgm, adc_ol::mAdcConverions);
                    break;
                default:
                    break;
                }
