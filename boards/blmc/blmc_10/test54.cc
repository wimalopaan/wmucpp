...
using commuteTimer = SimpleTimer<Component::Tcb<1>, std::integral_constant<uint8_t, 2>>; 
...
//            sensor::periodic();
            ...
            systemTimer::periodic([&]{
                ...
                if (controller::mState == controller::State::Sine) {
                    ...
                    fp = controller::mDesiredPeriod / 100;
                    controller::mActualPeriodLoop = 0;
                }
                else if (controller::mState == controller::State::ClosedLoop) {
                    
                    fp = (16 * fp + controller::mActualPeriodLoop / 100) / 17;
                    
                    if ((fp > 100) && (fp < 650)) {
                        if (fp > (controller::mDesiredPeriod / 100 + 50)){
//                            controller::pwmInc();
                        }
                        if (fp < (controller::mDesiredPeriod / 100 - 50)){
//                            controller::pwmDec();
...
//                sensor::ratePeriodic();
                alarmTimer::periodic([&](const auto& t){
                    if (t == periodicTimer) {
                        etl::outl<terminal>("p: "_pgm, controller::mActualPeriod.toInt(), " t: "_pgm, controller::mActualSineTable.toInt(), 
                                            " s: "_pgm, controller::mScale.value.toInt(), " apl: "_pgm, controller::mActualPeriodLoop, " dpl: "_pgm, controller::mDesiredPeriod,
                                            " fp: "_pgm, fp);            
