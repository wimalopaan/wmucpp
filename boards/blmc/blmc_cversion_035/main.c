...
void setup(void)
{
    ...
    myId = eeprom_read_byte((uint8_t*) EPRM_MID); // motor ID
    
    if(myId<1 || myId > 8)
    {
        myId = MOTOR_ID; // if invalid value, set it to default
    }
...
//    TIMSK =0;
    ...
    ACSR &= ~(1<<ADEN); //switch to comparator multiplexed
//    ADCSR &= ~(1<<ADEN); //switch to comparator multiplexed
    SFIOR |= (1<<ACME);
...
//    TIFR = TIMSK = (1<<TOIE1) +(1<<OCIE1A)+(1<<TOIE2); // Enable the 3 interrupts we use in the code
    TIFR = TIMSK0 = (1<<TOIE1) +(1<<OCIE1A)+(1<<TOIE2); // Enable the 3 interrupts we use in the code
    set_all_timings();
