...
using leds1 = WS2812<8, leds1Pin, ColorSequenceGRB>;
...
using terminal = BufferedStream<SSpi0, 128>;
...
using ppmTimerOutput = AVR::Timer16Bit<3>; // timer 3
...
struct DS18B20MeasurementHandler: public EventHandler<EventType::DS18B20Measurement> {
    static bool process(uint8_t) {
        ...
        Hott::SensorProtocollBuffer<0>::temp1(t.integer() + 20);
        return true;
    }
};
