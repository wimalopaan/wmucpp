...
using gpsUart = AVR::Usart<0, GPS::GpsProtocollAdapter<0>, MCU::UseInterrupts<false>, UseEvents<false>>;
using gpsterminal = std::basic_ostream<gpsUart>;
        ...
        systemTimer::periodic<systemTimer::flags_type::ocfa>([&](){
            alarmTimer::periodic([&](uint7_t timer) {
                if (timer == *periodicTimer) {
                    ...
                    std::outl<gpsterminal>("tick"_pgm);
                }
            });
        });
    }
