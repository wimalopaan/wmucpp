...
geladen werden. Es kommt in diesem Fall zu einer Fehlermeldung des Senders.
...
* `RES`: Reset aller Werte der Funktion
...
[CAUTION]
--
Leider ist der _Werte-Bereich_, in denen die einzelnen Kanäle eines SBus-Signal skaliert werden, nicht wohl definiert, und einige Hersteller 
legen unterschiedliche Grenzen des Wertebereiches fest. Dies führt zu Problemen!
