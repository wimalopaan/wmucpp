...
template<typename PWM, typename AN, typename BN, typename AN2, typename BN2>
struct Stepper5 {
    ...
    inline static constexpr uint16_t period= 1250; // 16KHz
    ...
    inline static void init() {
        ...
        AN2::low();
        BN2::low();
        AN2::template dir<Output>();
        BN2::template dir<Output>();
...
private:    
    template<typename P1, typename P2, auto N>
    static inline void forward() {
        PWM::template noinvert<AVR::PWM::WO<N>>();
//        lut0::init(std::byte{0xaa});     
//        lut1::init(std::byte{0xcc});     
        P1::low();
        P2::low();
    }
    template<typename P1, typename P2, auto N>
    ...
    inline static etl::uint_ranged_circular<uint16_t, 0, steps - 1> index2{0}; // adaptiv machen
};
...
using an2= Pin<PortA, 6>; // Pin 4
using an = Pin<PortA, 5>; // Pin 3 
//using bp = Pin<PortA, 6>; // 4 
using bn2 = Pin<PortB, 3>; // 6 
using bn = Pin<PortB, 2>; // 7
...
using stepper = Stepper5<pwm, an, bn, an2, bn2>;
...
using portmux = Portmux::StaticMapper<Meta::List<usart0Position, tcaPosition, ccl0Position, ccl1Position>>;
