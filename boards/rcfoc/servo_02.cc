...
//    m_absdiff_t::_;
    ...
    // 3510
    using driver = Driver<typename devs::pwm, std::integral_constant<size_t, encoder::steps>, std::integral_constant<uint8_t, 11>, m_absdiff_t>;
...
//    using driver = Driver<typename devs::pwm, std::integral_constant<size_t, 2048>, std::integral_constant<uint8_t, 7>, m_absdiff_t>;
    ...
    static inline constexpr m_angle_t zeroAngle{encoder::steps / 2 - 1};
...
//        encoder::read(); 
    ...
    static inline void ratePeriodic() {
        ...
        (++mDebugTick).on(debugTicks, [&]{
            etl::outl<terminal>("ma: "_pgm, angle.toInt(), " eo: "_pgm, eoffset.toInt(), " target: "_pgm, target.toInt(),
        ...
        case State::ZeroEnd:
            ...
            break;
        case State::Run:
            if (const auto ch0 = servo_pa::value(0); ch0) {
                const auto a0 = AVR::Pgm::scaleTo<m_diff_t>(ch0.toRanged());
...
private:
    ...
    static inline m_angle_t target{0};
...
struct Application {
    ...
    inline static void run(const bool inverted = false) {
        if constexpr(External::Bus::isSBus<BusSystem>::value || External::Bus::isIBus<BusSystem>::value) {
            ...
            etl::outl<terminal>("Servo_02"_pgm);
