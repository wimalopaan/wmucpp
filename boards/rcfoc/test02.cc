...
struct Driver {
    ...
    static inline constexpr uint16_t steps = 600;
    
    static inline constexpr uint8_t scale = 2;
...
private:    
    inline static void setDuty() {
        PWM::template duty<Meta::List<AVR::PWM::WO<0>>>((scale * sine1[index0]) / 10);
        PWM::template duty<Meta::List<AVR::PWM::WO<1>>>((scale * sine1[index1]) / 10);
        PWM::template duty<Meta::List<AVR::PWM::WO<2>>>((scale * sine1[index2]) / 10);
    }
