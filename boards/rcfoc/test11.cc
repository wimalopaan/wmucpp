...
//    using sensor_pa = Sensor::ProtocollAdapter;
    ...
    enum class State : uint8_t {Undefined, Init, Enable, AlignStart, Aligned};
...
//        const auto d = cyclic_diff(angle_t{0}, angle);
        
        (++mDebugTick).on(debugTicks, [&]{
            etl::outl<terminal>("ma: "_pgm, angle.toInt(), " ea: "_pgm, eoffset.toInt());
//            etl::outl<terminal>("ma: "_pgm, angle.toInt(), " ea: "_pgm, eoffset.toInt(), " d: "_pgm, d);
        });
...
//            }
        ...
        {
            if (angle >= off) {
                const angle_t x(uint16_t(angle - off));
                driver::angleMaxTorqueLeft(x);
            }
            else {
                const angle_t x(angle - off + angle_t::Upper);
                driver::angleMaxTorqueLeft(x);
        ...
        if (oldState != mState) {
            ...
            case State::AlignStart:
                driver::angle(eoffset);
                etl::outl<terminal>("S: AlStart"_pgm);
                break;
            case State::Aligned:
                etl::outl<terminal>("S: Aligned"_pgm);
                break;
            case State::Enable:
...
//    std::integral_constant<uint16_t, piHalf>::_;
    ...
    static inline void angleMaxTorqueLeft(const index_type& a) {
        uint16_t i = a + 10;
        if (i >= steps) {
            i -= steps;    
        }
...
struct Devices {
    ...
    using sbus = External::SBus::Output::Generator<usart2Position>;
...
struct Application {
    ...
    inline static void run(const bool inverted = false) {
        if constexpr(External::Bus::isSBus<BusSystem>::value || External::Bus::isIBus<BusSystem>::value) {
            ...
            etl::outl<terminal>("foc_t10_hw01"_pgm);
