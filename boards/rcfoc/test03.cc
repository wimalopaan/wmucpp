...
//    using pwm = devs::pwm;
    ...
    static inline void ratePeriodic() {
        ...
        encoder::startRead();
        const auto a = encoder::readLast();
        
        (++mDebugTick).on(debugTicks, [&]{
            if (a) {
                etl::outl<terminal>("*: "_pgm, *a);
            }
            else {
    ...
    struct Generator {
        constexpr auto operator()() {
            ...
            for(uint16_t i = 0; i < steps; ++i) {
                data[i] = (double)(period - 1) * (1.0 + sin((i * 2.0 * M_PI) / steps)) / 2.0;
            }
            return data;
        }
    };
    using Sine1 = AVR::Pgm::Util::Converter<Generator>::pgm_type;
...
private:    
    inline static void setDuty() {
        PWM::template duty<Meta::List<AVR::PWM::WO<0>>>((scale * Sine1::value(index0)) / 10);
        PWM::template duty<Meta::List<AVR::PWM::WO<1>>>((scale * Sine1::value(index1)) / 10);
        PWM::template duty<Meta::List<AVR::PWM::WO<2>>>((scale * Sine1::value(index2)) / 10);
    }

    inline static etl::uint_ranged_circular<uint16_t, 0, steps - 1> index0{0};
    inline static etl::uint_ranged_circular<uint16_t, 0, steps - 1> index1{(steps / 3)};
    inline static etl::uint_ranged_circular<uint16_t, 0, steps - 1> index2{ (2 * steps) / 3};
...
//            spi::put(Command{Register::NOP, true});
        }
        
        static inline std::optional<uint16_t> readLast() {
            if (spi::rxQueue().size() < 1) {
                return {};
            }
            else {
//                spi::rxQueue().pop_front();
...
struct Devices {
    ...
    using spi = AVR::SpiBiSS<spiPosition, Command, Result, AVR::QueueLength<128>, ssPin>;
