...
//    using pwm = devs::pwm;
    ...
    static inline void ratePeriodic() {
        ...
        encoder::startRead();
        const uint16_t angle = encoder::angle();
        
        (++mDebugTick).on(debugTicks, [&]{
            etl::outl<terminal>("*: "_pgm, angle, " u: "_pgm, encoder::u, " l: "_pgm, encoder::l);
        });
...
//    static inline constexpr uint16_t steps = 48;
...
namespace External::AS5147 {
    ...
    struct Encoder {
        ...
        static inline void startRead() {
            mStarted = spi::put(Command{Register::ANGLECOM, true}, Command{Register::NOP, false});
        }
        ...
        static inline void periodic() {
            spi::periodic();
            u = uint8_t(spi::result()[0] & 0x3f_B);
            l = uint8_t(spi::result()[1]);
            mAngle = (uint16_t(u) << 8) | l;
        }
        
        static inline const uint16_t& angle() {
            return mAngle;
        }
//    private: 
        static inline uint8_t u;
        static inline uint8_t l;
        static inline uint16_t mAngle{};
        static inline bool mStarted{false};
    };
}
...
struct Devices {
    ...
    using spi = AVR::SpiBiSS2<spiPosition, Command, Result, AVR::QueueLength<2>, ssPin>;
