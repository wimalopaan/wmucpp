...
namespace Constants {
    ...
    static constexpr auto title = "Test 10"_pgm;
...
struct I2CInterrupt : public IsrBaseHandler<AVR::ISR::Int<1>> {
    static void isr() {
        EventManager::enqueue({EventType::ExternalInterrupt, 1_B});
    }
};
...
struct DS18B20MeasurementHandler: public EventHandler<EventType::DS18B20Measurement> {
    static bool process(std::byte) {
        ...
        if (tempFSM::mSensorNumber == 0) {
            sensorData::temp1(t);
            tempFSM::mSensorNumber = 1;
        }
        else {
            sensorData::temp2(t);
            tempFSM::mSensorNumber = 0;
        }
...
struct TimerHandler : public EventHandler<EventType::Timer> {
    inline static bool process(std::byte b) {
        ...
        if (timer == *periodicTimer) {
            ...
            std::outl<terminal>("rpm1: "_pgm, upm1.value());
