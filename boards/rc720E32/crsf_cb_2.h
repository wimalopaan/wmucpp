...
struct CrsfCallback {
    ...
    using Param_t = RC::Protokoll::Crsf::V4::Parameter<uint16_t>;
    ...
    static inline void setParameterValue(const uint8_t index, const auto data, const uint8_t paylength) {
        IO::outl<debug>("# SetPV i: ", index);
        if ((index >= 1) && (index <= params.size())) {
            mLastChangedParameter = index;
            bool mustSave = true;
            if (params[index - 1].type == Param_t::Str) {
                IO::outl<debug>("# String");
                if (params[index - 1].stringValue) {
                    for(uint8_t i = 0; (i < 16) && (i < paylength); ++i) {
                        params[index - 1].stringValue[i] = data[i];
                        if (data[i] == '\0') {
                            break;
                        }
                    }
                    params[index - 1].stringValue[paylength] = '\0';
                }
            }
            else {
                Param_t::value_type value{};
                if (params[index - 1].type <= Param_t::I8) {
                    value = data[0];
                    IO::outl<debug>("# I8: v: ", value);
                }
                else if (params[index - 1].type <= Param_t::I16) {
                    value = (data[0] << 8) + data[1];
                    IO::outl<debug>("# I16: v: ", value);

                }
                else if (params[index - 1].type <= Param_t::F32) {
                    value = (data[3] << 24) + (data[2] << 16) + (data[1] << 8) + data[0];
                    IO::outl<debug>("# F32: v: ", value);
                }
                else if (params[index - 1].type == Param_t::Sel) {
                    value = data[0];
    ...
    static inline bool isCommand(const uint8_t index) {
        if ((index >= 1) && (index <= params.size())) {
            return params[index - 1].type == PType::Command;
        }
        return false;
    }
...
private:
    ...
    using params_t = etl::FixedVector<Param_t, 96>;
    static inline params_t params = []{
        ...
        addNode(p, Param_t{.parent = parent, .type = PType::Sel, .name = "Stream", .options = "Main/CRSF;Alternative;Aux", .value_ptr = &eeprom.input_stream, .max = 2, .cb = [](const uint16_t s){mapper::stream(s); return true;}});
        addNode(p, Param_t{.parent = parent, .type = PType::U8, .name = "Schottel 1: f/b", .value_ptr = &eeprom.channels[0].first, .max = 15, .cb = [](const uint16_t){return true;}});
        addNode(p, Param_t{.parent = parent, .type = PType::U8, .name = "Schottel 1: l/r", .value_ptr = &eeprom.channels[0].second, .max = 15, .cb = [](const uint16_t){return true;}});
        addNode(p, Param_t{parent, PType::U8, "Schottel 2: f/b", nullptr, &eeprom.channels[1].first, 0, 15, [](const uint16_t){return true;}});
        addNode(p, Param_t{parent, PType::U8, "Schottel 2: l/r", nullptr, &eeprom.channels[1].second, 0, 15, [](const uint16_t){return true;}});
        ...
        addNode(p, Param_t{.parent = parent, .type = PType::U8, .name = "Sch1 ESC Deadband", .value_ptr = &eeprom.deadbands[0], .min = 1, .max = 20, .cb = [](const uint16_t){return true;}, .unitString = "%"});
        ...
        addNode(p, Param_t{parent, PType::U16,  "Sch2 Servo zPosition", nullptr,  &eeprom.offset2, 0, 359, [](const uint16_t v){servos::template offset<1>((uint32_t{v} * 4096) / 360); return true;}});
        addNode(p, Param_t{parent, PType::U16,  "Sch2 Servo Speed", nullptr, &eeprom.speed2, 1, 340, [](const uint16_t v){servos::template speed<1>(10 * v); return true;}});
        ...
        addNode(p, Param_t{parent, PType::U16,  "CRSF Address", nullptr, &eeprom.address, 192, 207, [](const uint16_t a){updateName(mName); src::address(std::byte(a)); return true;}});
