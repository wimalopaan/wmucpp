...
eine andere Festlegung treffen. Daher ist es nur wichtig festzulegen, welche Kanal der _erste_ Kanal dieser Gruppe sein soll, die anderen Kanäle ergeben sich dann 
...
Kanal stem:[(S_0 + 4)]:: Kalibrierungskanal 1 (oder Hilfskanal 1)
Kanal stem:[(S_0 + 5)]:: Kalibrierungskanal 2 (oder Hilfskanal 2)
...
Drehrichtung des Propellers niemal umgekehrt wird. Der am `RC-720-D` eingesetzte Steller muss jedoch trotzdem ein V/R-Steller sein. Dieser wird allerdings nur in _Vorwärtsrichtung_ 
...
Schubvektor 2 definiert. Zusätzlich wird der _Input_ `Gas` auch noch für die beiden Kalibrierungskanäle `12` und `15` eingesetzt.
...
Um in den _Konfigurationsmodus_ zu gelangen, stecken Sie auf den Anschluß `O1` am `RC-720-D`-Modul eine Brücke zwischen den Pins `2` und `3` der
3-poligen Stiftleiste (die kann durch ein Dupont-Jumper-Kabel geschehen oder auch einfach nur durch eine metallische Brücke).
...
.. Wurde die Neuralposition erfolgreich erkannt, so leuchtet zusätzlich zu LED 2 nun auch wieder die LED 1 für ca. 1 Sekunde auf.
...
. Nachdem auch die Kalibrierungsphase für beide Servos abgeschlossen ist, geht das `RC-720-D` in den _Betriebsmodus_ über. Anhand der Servos sollten Sie nun 
...
Das `RC-720-D` benutzt die _physische_ Sensor-ID `1` und stellt die folgenden Sensoren zur Verfügung:
...
.Das Telemetrie-Widget: es sind hier _zwei_ Instanzen des Schottel-Telemetrie-Widgets konfiguriert. 
