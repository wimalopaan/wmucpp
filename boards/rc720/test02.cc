    ...
    static inline void ratePeriodic() {
        ...
        mDebugTick.on(debugTicks, []{
            etl::outl<terminal>("ch0: "_pgm, servo_pa::value(0).toInt(), 
                                " a0: "_pgm, adc::value(adc_i_t{0}).toInt(), " a1: "_pgm, adc::value(adc_i_t{1}).toInt(), " a2: "_pgm, adc::value(adc_i_t{2}).toInt()
                                );
#ifndef NDEBUG
            if (xassert::on) {
        ...
        case State::Calibrate:
            ...
            break;
        case State::Run:
            const auto dir = servo_pa::value(searchCh + 1);
            servo_0::position(dir);
...
struct Application {
    ...
    inline static void run(const bool inverted = false) {
        if constexpr(External::Bus::isSBus<BusSystem>::value || External::Bus::isIBus<BusSystem>::value) {
            ...
            etl::outl<terminal>("rc720_02"_pgm);
