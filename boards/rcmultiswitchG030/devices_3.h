...
#ifdef USE_MORSE
    static inline std::array<char, sizeof(storage::eeprom.morse_texts)> morse_text{};
    struct BConfig {
        using timer = systemTimer;
        using debug = Devices::debug;
        static inline constexpr auto& text = morse_text;
    };
...
#ifdef USE_MORSE
        static inline constexpr auto& morse_text = Devices::morse_text;
#endif
    };
    struct CrsfConfig {
        using txpin = crsftx;
