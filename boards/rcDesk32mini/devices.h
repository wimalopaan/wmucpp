...
struct Devices<WeAct_SFrog, Config, MCU> {
...
# warning "wrong protocol selection"
    ...
    using led1 = Mcu::Stm::Pin<gpioa, 3, MCU>;
    using ledBlinker1 = External::Blinker<led1, systemTimer>;
    using led2 = Mcu::Stm::Pin<gpioa, 8, MCU>;
    using ledBlinker2 = External::Blinker<led2, systemTimer>;

#ifdef SERIAL_DEBUG
    using analogs = Meta::List<vin0, vin1, vin4, vin5, vin6>;
#else
    using analogs = Meta::List<vin0, vin1, vin2, vin4, vin5, vin6>;
#endif
    struct AdcConfig;
    ...
    using btn0 = External::Button<d0, systemTimer, External::Tick<systemTimer>{300ms}.raw(),
                                  External::Tick<systemTimer>{3000ms}.raw(), void>;
    using btn1 = External::Button<d1, systemTimer, External::Tick<systemTimer>{300ms}.raw(),
                                  External::Tick<systemTimer>{3000ms}.raw(), void>;
    using btn2 = External::Button<d2, systemTimer, External::Tick<systemTimer>{300ms}.raw(),
                                  External::Tick<systemTimer>{3000ms}.raw(), void>;
    using btn3 = External::Button<d3, systemTimer, External::Tick<systemTimer>{300ms}.raw(),
                                  External::Tick<systemTimer>{3000ms}.raw(), void>;
    ...
    struct AdcConfig {
        using debug = void;
#ifdef SERIAL_DEBUG
        using channels = std::integer_sequence<uint8_t, 0, 1, 5, 6, 7>;
#else
        using channels = std::integer_sequence<uint8_t, 0, 1, 2, 5, 6, 7>;
#endif
    ...
    static inline bool press1() {
        if (btn0::event() == btn0::Press::Short) return true;
        if (btn1::event() == btn1::Press::Short) return true;
        if (btn2::event() == btn2::Press::Short) return true;
        if (btn3::event() == btn3::Press::Short) return true;
        return false;
    }
    static inline bool bootPress() {
        return (!d1::read() && !d2::read()) || (!d0::read() && !d3::read());
    }
    static inline bool bootRelease() {
        return (d1::read() && d2::read() && d0::read() && d3::read());
    }
    ...
    static inline void init() {
        ...
        led1::set();
        led2::set();
        ledBlinker1::event(ledBlinker1::Event::Medium);
        ledBlinker2::event(ledBlinker2::Event::Medium);
