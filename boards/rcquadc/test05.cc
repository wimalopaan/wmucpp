...
namespace Storage {
    ...
    struct Output {
        ...
        External::Tick<Timer> currentCheckTimeout{3000_ms};
...
struct ChannelFsm {
    ...
    enum class Event : uint8_t {None, Stop, Forward, Backward, SetFowardPwm, SetBackwardPwm, 
                                SetForwardOC, SetBackwardOC,
//                                SetForwardPwmReleaseBackward, SetBackwardPwmReleaseForward
                               };
...
//    std::integral_constant<uint16_t, currentCheckCycles>::_;
    ...
    static inline auto& pwmBackward() {
        return NVM::data().output(data_index).pwmBackward;        
    }
    ...
    inline static auto& rampCycles() {
        return NVM::data().output(data_index).rampCyles;
    }
...
//            mEvent = Event::SetForwardPwmReleaseBackward;
        }
    }
...
//            mEvent = Event::SetBackwardPwmReleaseForward;
        }
    }

    inline static void setOffCurrentForward(const auto v) {
        static std::remove_cvref_t<decltype(v)> lastV;
        using curr_t = std::remove_cvref_t<decltype(forwardOffCurr())>;
        if (v.isTop()) {
            forwardOffCurr() = std::numeric_limits<curr_t>::max();            
    ...
    inline static void setOffCurrentBackward(const auto v) {
        static std::remove_cvref_t<decltype(v)> lastV;
        using curr_t = std::remove_cvref_t<decltype(backwardOffCurr())>;
        if (v.isTop()) {
            backwardOffCurr() = std::numeric_limits<curr_t>::max();                        
    ...
    static inline void debug() {
        etl::out<Term>("Ch"_pgm, N, " State: "_pgm, uint8_t(mState));
        etl::out<Term>(" to:"_pgm, ccTimeout().value, " rc: "_pgm, rampCycles().value);
        etl::out<Term>(" fpw: "_pgm, pwmForward(), " bpw: "_pgm, pwmBackward());
        etl::out<Term>(" lfc: "_pgm, mLastForwardCurrent, " foc: "_pgm, forwardOffCurr());
        etl::outl<Term>(" lbc: "_pgm, mLastBackwardCurrent, " boc: "_pgm, backwardOffCurr());
    }
