...
//    static inline constexpr channel_t channel{N};
    ...
    enum class State : uint8_t {Init, Off, OffWait, Forward, Backward, ForwardWait, BackwardWait, 
...
//    std::integral_constant<uint16_t, currentCheckTimeout.value>::_;
    ...
    static inline pwm_value_t mPwmForward{PWM::pwmMax / 4};
    static inline pwm_value_t mPwmBackward{PWM::pwmMax / 4};
    static inline pwm_value_t mRampPwm{};
    
    static inline uint16_t mLastForwardCurrent{};
    static inline uint16_t mLastBackwardCurrent{};

    static inline uint16_t mForwardOffCurrent{std::numeric_limits<uint16_t>::max()};
    static inline uint16_t mBackwardOffCurrent{std::numeric_limits<uint16_t>::max()};
...
//        decltype(v)::_;
        ...
        mPwmForward.set(((uint16_t)PWM::pwmMax * (v - v_t::Lower)) / (v_t::Upper - v_t::Lower));
        mLastForwardCurrent = lcv;
        mForwardOffCurrent = 2 * mLastForwardCurrent;
        mEvent = Event::SetFowardPwm;
    }
    inline static void setPwmBackward(const auto v) {
//        decltype(v)::_;
        using v_t = std::remove_cvref_t<decltype(v)>;
        mPwmBackward.set(((uint16_t)PWM::pwmMax * (v - v_t::Lower)) / (v_t::Upper - v_t::Lower));
        mLastBackwardCurrent = lcv;
        mBackwardOffCurrent = 2 * mLastBackwardCurrent;
...
//        decltype(v)::_;
//        using v_t = std::remove_cvref_t<decltype(v)>;
        mForwardOffCurrent = mLastForwardCurrent + ((mLastForwardCurrent * v.toInt()) >> 3);
        mEvent = Event::SetForwardOC;
    }
    inline static void setOffCurrentBackward(const auto v) {
//        decltype(v)::_;
//        using v_t = std::remove_cvref_t<decltype(v)>;
        mBackwardOffCurrent = mLastBackwardCurrent + ((mLastBackwardCurrent * v.toInt()) >> 3);
    ...
    inline static void ratePeriodic() {
        ...
        if (oldState != mState) {
            ...
            case State::EEPromWrite:
                ...
                break;
            case State::Debug1:
                etl::out<Term>("State: "_pgm, uint8_t(Meta::front<channel_list>::mState));
                etl::out<Term>(" fpw: "_pgm, Meta::front<channel_list>::mPwmForward, " bpw: "_pgm, Meta::front<channel_list>::mPwmBackward);
                etl::out<Term>(" lfc: "_pgm, Meta::front<channel_list>::mLastForwardCurrent, " foc: "_pgm, Meta::front<channel_list>::mForwardOffCurrent);
                etl::outl<Term>(" lbc: "_pgm, Meta::front<channel_list>::mLastBackwardCurrent, " boc: "_pgm, Meta::front<channel_list>::mBackwardOffCurrent);
                break;
            }
        }
    }
