...
struct CrsfCallback {
    ...
    static inline constexpr const char* const title = "BDC";
    ...
    static inline void update() {
        ...
        std::to_chars(std::begin(mMaxRpmString), std::end(mMaxRpmString), maxRpm * 100);
        const uint16_t maxPwm = eeprom.cutoff_freq * eeprom.n_fsample * eeprom.subsampling;
        std::to_chars(std::begin(mPwmFreqString), std::end(mPwmFreqString), maxPwm * 100);
    }
    ...
    static inline params_t params = []{
        ...
        addNode(p, Param_t{0, PType::U8,  "Inertia", nullptr, &eeprom.inertia, 0, 9, [](const uint8_t v){update(); return true;}});
        addNode(p, Param_t{0, PType::Info, "PWM Freq", &mPwmFreqString[0]});
        addNode(p, Param_t{0, PType::Info, "Max RPM", &mMaxRpmString[0]});
        addNode(p, Param_t{0, PType::Sel,  "Calibrate", "Off;Start", nullptr, 0, 1, [](uint8_t){return false;}});
        return p;
    }();
};
