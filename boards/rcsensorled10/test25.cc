...
private:
    ...
    static inline void update1() {
        if (const auto& s = appData[Storage::AVKey::TSensor1]) {
            sensorData::temp1(Util::select(s.toInt(), Storage::temps[0], Storage::temps[1], Storage::temps[2], Storage::temps[3]));
        }
        if (const auto& s = appData[Storage::AVKey::TSensor2]) {
            sensorData::temp2(Util::select(s.toInt(), Storage::temps[0], Storage::temps[1], Storage::temps[2], Storage::temps[3]));
        }
    }
...
//                        std::outl<terminal>("W: "_pgm, hx711::value());
                        ...
                        if (eepSave) {
                            ...
                            std::outl<terminal>("eeprom save"_pgm);
                        }
# ifdef MEM
