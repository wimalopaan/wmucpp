...
struct GlobalFsm {
    ...
    static void init() {
        ...
        devs::lut2::init(std::byte{0x0f}); // route TXD (inverted) to lut3-out         
        devs::lut2::enable();
        devs::sensor::init();
...
//                etl::outl<terminal2>("serial2: "_pgm, devs::sbus_pa::value(0).toInt());                
                etl::outl<terminal2>("serial2: "_pgm, devs::sbus_pa::packages(), " : "_pgm, devs::sbus_pa::value(0).toInt(), " : "_pgm, devs::sensor::ProtocollAdapter::requests());                
            });
