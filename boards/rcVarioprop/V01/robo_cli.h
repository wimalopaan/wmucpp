...
namespace External {
    namespace RoboCli {
        ...
        constexpr auto buttonSymbol = std::byte{'b'};
        ...
        template<uint8_t N, typename Timer, typename PassThru, typename SendBack = void, typename CallBack = void, uint8_t NChannels = 16, typename Buffer = void, typename Dbg = void>
        struct ProtocollAdapter final {
            ...
            enum class Target: uint8_t {Undefined, Prop, Switch, Toggle, Ping, Wind, Menu, ButtonPress};
            ...
            inline static bool process(const std::byte b) {
                ...
                case State::Start:
                    ...
                    else if (b == buttonSymbol) {
                        state = State::Number;
                        target = Target::ButtonPress;
                    }
                    else {
                        state = State::Undefined;
                        target = Target::Undefined;
                ...
                case State::Number:
                    if (isDigit(b)) {
                        ...
                        else if (target == Target::ButtonPress) {
                            state = State::Undefined;
                            lastIndex.setNaN();
                            lastTarget = Target::Undefined;
                            CallBack::buttonPress(number);
                        }
                        else {
                            state = State::Undefined;
                        }
