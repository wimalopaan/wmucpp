...
struct GFSM {
    ...
    struct CrsfTransmitter {
        enum class State : uint8_t {Init, Channels, Switches};

        static inline void ratePeriodic() {
            switch(mState) {
            case State::Init:
                Gen::target(RC::Protokoll::Crsf::Address::TX);
                mState = State::Channels;
                break;
            case State::Channels:
                Gen::channels(Data::channels);
                mState = State::Switches;
                break;
            case State::Switches:
                mState = State::Channels;
                break;
    ...
    using crsfTransmitter = CrsfTransmitter<crsf_out>;
    ...
    static inline constexpr External::Tick<systemTimer> crsfTicks{10ms};
    ...
    static inline void ratePeriodic() {
        ...
        case State::Run:
            ...
            if (mCrsfReceiver) {
                ...
                (++mCrsfTick).on(crsfTicks, []{
                    crsfTransmitter::ratePeriodic();
                });
            }
    ...
    static inline void updateAnalogs() {
        for(uint8_t i = 0; i < 4; ++i) {
            if (mCrsfReceiver && Data::crsfOverride) {
    ...
    static inline External::Tick<systemTimer> mCrsfTick;
    static inline bool mCrsfReceiver{true};
};
