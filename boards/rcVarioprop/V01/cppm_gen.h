...
struct Generator {
    ...
    static inline constexpr uint16_t frame = 22.5f * onems;
    ...
    static inline void init() {
        ...
        mcuTimer->ARR = onems + halfms;
        mcuTimer->CCR1 = halfms;
    ...
    static inline void set(uint8_t channel, uint16_t value) {
        ...
        arr[channel] = value + onems;
        uint16_t sum = arr[0];
        for(uint8_t i = 1; i < channels; ++i) {
            sum += arr[channel];
        }
        arr[pulses - 1] = frame - sum;
    }
private:
    static inline std::array<volatile uint16_t, pulses> arr;
};
}
}
