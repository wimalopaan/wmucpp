...
//using twi = Twi::Master<twi0Position, c0Pin, c1Pin>;
...
namespace External {
    namespace SI5351 {
        ...
        struct Clock {
            ...
            static inline bool read(const uint8_t regNr, std::byte& data) {
                if (mState != State::Idle) {
                    return false;
                }
                std::array<std::byte, 1> a{std::byte{regNr}};
                if (!bus::write(Adr, a)) {
                    return false;
                }
                bus::startReadByte(Adr);
                return true;
            }
            ...
            static inline void periodic() {
                ...
                case State::DefaultSetup1:
                    if (twi::write(si5351, {SI5351_REGISTER_3_OUTPUT_ENABLE_CONTROL, 0xFF})) {
                        mState = State::DefaultSetup2;
                        iindex = 0;
                    }
                    break;
                case State::DefaultSetup2:
                      if (twi::write(si5351, m_si5351_regs_15to92_149to170[iindex])) {
                          ++iindex;
                ...
                case State::DefaultSetup3:
                    if (twi::write(si5351, {SI5351_REGISTER_177_PLL_RESET, 0xAC})) {
                        mState = State::DefaultSetup4;
                    }
                    break;
                case State::DefaultSetup4:
                    if (twi::write(si5351, {SI5351_REGISTER_3_OUTPUT_ENABLE_CONTROL, 0x00})) {
            ...
            static inline bool off() {
                return twi::write(si5351, {SI5351_REGISTER_3_OUTPUT_ENABLE_CONTROL, 0xff});
            }
            
            static inline bool on() {
                return twi::write(si5351, {SI5351_REGISTER_3_OUTPUT_ENABLE_CONTROL, 0x00});
            }
...
struct Fsm {
    enum class State : uint8_t {Undefined, Init, Set, Run, PreRead, Read};
