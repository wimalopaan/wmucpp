...
struct Fsm {
    ...
    enum class State : uint8_t {Undefined, Init, Set, Search, Run, RunRF};
...
//        dbgPin::dir<Output>();
        ...
        etl::outl<terminal>("rfmodule 01"_pgm);
    }
    static inline void periodic() {
//        dbgPin::toggle();
        led::periodic();
...
//                etl::out<terminal>(" v"_pgm, i, ": "_pgm, mpm_pa::value(i).toInt());
//            }
//            etl::outl<terminal>();
            mpm_out::channel(mpm_pa::channel());
//            ad9851::channel(mpm_pa::subProto(), mpm_pa::channel());
        });
        switch(mState) {
...
//            for(uint8_t i{0}; i < 8; ++i) {
//                const auto v = mpm_pa::value(i);
//                if (const auto vr = v.toRanged(); v) {
...
//            });
        ...
        case State::Run:
            ...
            switch(mpm_pa::subProto()) {
            case 0:
                mpm_out::band(mpm_out::Band::_27MHz);
                break;
            case 1:
                mpm_out::band(mpm_out::Band::_35AMHz);
                break;
            case 2:
                mpm_out::band(mpm_out::Band::_35BMHz);
                break;
            case 3:
                mpm_out::band(mpm_out::Band::_40MHz);
                break;
            case 4:
                mpm_out::band(mpm_out::Band::_41MHz);
                break;
            case 5:
                mpm_out::band(mpm_out::Band::_72MHz);
                break;
            }
            if (mpm_pa::bind()) {
                mState = State::RunRF;
