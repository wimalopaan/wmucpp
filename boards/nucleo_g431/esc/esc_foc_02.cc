...
//                IO::out<term>(" [", meas::mHallSectionLengths[sIndex].lastLength, ",", meas::mHallSectionLengths[sIndex].sineIndex, "]"); 
                IO::out<term>(" [", meas::mHallSectionLengths[sIndex].lastLength, "]"); 
                ++sIndex;
                if (sIndex == (6 * 7)) {
...
//                IO::outl<term>("s[0]: ", Storage::adcValues[0], " s[1]: ", Storage::adcValues[1], " s[2]: ", Storage::adcValues[2]);
                IO::outl<term>("v: ", (uint16_t)(1000 * pos::Interrupt::expMean.value()));
                IO::outl<term>("m: ", meas::mRotation, " s: ", meas::mMechSection);
                IO::outl<term>("ds: ", (uint16_t)(1000 * driver::mScale), " af: ", (uint16_t)(1000 * pos::Interrupt::mAngleFaktor));
            });
            if (const auto input = term::get()) {
                ...
                break;
                case '+':
                    pos::Interrupt::mAngleFaktor *= 1.1;
                break;
                case '-':
                    pos::Interrupt::mAngleFaktor *= 0.9;
                break;
                default:
                break;
                }
...
int main() {
    ...
    NVIC_SetPriority(TIM4_IRQn, 0); // highest: Hall
    NVIC_SetPriority(TIM7_IRQn, 1); // estimator
    NVIC_SetPriority(TIM6_DAC_IRQn, 2); // measurement
    NVIC_SetPriority(DMA1_Channel1_IRQn, 3); // DMA
