    ...
    namespace V4 {
        ...
        struct Adc<N, Config, MCU> {
            ...
            static inline void init() {
                ...
                mcuAdc->CFGR = [] consteval {
                    uint32_t r = 0;
                    r |= (0x00 << ADC_CFGR_RES_Pos);
                    if constexpr(isTriggerSource<trigger>) {
                        r |= (trigger::trgo() << ADC_CFGR_EXTSEL_Pos);
                        r |= (0b10UL << ADC_CFGR_EXTEN_Pos); // falling
                    }
                    if constexpr(isContinousSampling<trigger>::value) {
                        r |= ADC_CFGR_CONT;
                    }
                    r |= (ADC_CFGR_DMACFG | ADC_CFGR_DMAEN);
                ...
                //     MODIFY_REG(mcuAdc->CFGR , ADC_CFGR_EXTSEL_Msk, (trigger::trgo() << ADC_CFGR_EXTSEL_Pos));
                ...
                mcuAdc->SQR1 = []<auto... CC>(std::integer_sequence<auto, CC...>) consteval{
                ...
                if constexpr(nChannels > 4) {
                    mcuAdc->SQR2 = []<auto... CC>(std::integer_sequence<auto, CC...>) consteval{
                        uint32_t r = 0;
                        const std::array<uint8_t, sizeof...(CC)> channels{CC...};
