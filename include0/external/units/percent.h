...
namespace External::Units {
    ...
    constexpr bool operator==(percent lhs, percent rhs) {
    ...
    template<etl::Concepts::Integral T>
    constexpr T operator*(percent lhs, const T& rhs) {
        return (etl::enclosing_t<T>{rhs} * lhs.value()) / T{100};
    }
    
    namespace literals {
        constexpr percent operator"" _ppc(unsigned long long v) {
            return percent{static_cast<uint8_t>(v)};
        }
    }
    ...
    public:
        inline static constexpr percent scale(T value) {
            return percent{(uint8_t)(scale_r<0>(value - min))};
        }
    };
