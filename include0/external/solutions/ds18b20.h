...
namespace External {
    ...
    public:
        ...
        static constexpr std::byte family{0x28};
        ...
        typedef std::array<std::byte, readScratchpadSize> ds18b20_rsp_t;
        typedef std::array<std::byte, writeScratchpadSize> ds18b20_wsp_t;
        ...
        inline static void command(OneWire::Command cmd) {
            OneWireMaster::put(static_cast<std::byte>(cmd));
        }
        ...
        inline static FixedPoint<int16_t, 4> temperature(ds18b20_rsp_t& sp) {
            uint16_t valueL = static_cast<uint8_t>(sp[0]);
            uint16_t valueH = static_cast<uint8_t>(sp[1]);
